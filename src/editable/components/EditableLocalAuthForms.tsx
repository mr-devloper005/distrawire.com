...
const inputClass = 'h-[3.5rem] rounded-[16px] border border-[var(--slot4-border)] bg-[var(--slot4-warm)] px-4 text-base font-bold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-muted-text)]/70 focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-[3.5rem] items-center justify-center rounded-[16px] bg-[var(--slot4-dark-bg)] px-6 text-xs font-black uppercase tracking-[0.2em] text-white transition hover:brightness-110 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[16px] border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
